source("../../../src/analyzer/move2_mt_loc.R", chdir = TRUE)
#source("src/analyzer/move2_mt_loc.R", chdir = TRUE)
library(testthat)

test_data <- readRDS(file = "../data/move2_mt_loc/input2_mt_whitefgeese.rds")
#test_data <- readRDS(file = "tests/testthat/data/move2_mt_loc/input2_mt_whitefgeese.rds")

test_that("non-empty-result", {
  actual <- analyzeMove2MtLoc(rds = test_data)
  expect_equal(analyzeMove2MtLoc(rds = test_data)$n[1], "non-empty-result")
})
...
test_that("null-result", {
  actual <- analyzeMove2MtLoc(readRDS("../data/move2_mt_loc/input0_mt_null.rds"))
