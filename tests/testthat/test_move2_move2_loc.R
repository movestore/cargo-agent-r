...
test_that("attribs", {
  ...
  # expect_equal(length(actual$track_attributes),5)
  expect_equal(length(actual$track_attributes),length(names(unique(mt_track_data(test_data))[,!sapply(mt_track_data(test_data), function(x) all(is.na(x)))])))
  # expect_equal(length(actual$event_attributes),16)
  expect_equal(length(actual$event_attributes),length(names(test_data[,!sapply(test_data, function(x) all(is.na(x)))])))
})
